...
Invoke `requirements-grilling` for the interview and its completion gate. Invoke `domain-modeling` only if the interview resolves a reusable canonical term or exposes a genuine architecture candidate under that skill's triggers. Do not invoke it merely because a requirement uses domain language.
