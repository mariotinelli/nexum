...
Use this path after preflight and explicit selection of `new-scope`. Orchestration stays here; each item uses the reusable discipline in `new-issue.md` and its Feature or Bug branch.
...
Use one user journey as the Feature boundary: one primary actor intention ending in one observable outcome that the user can demonstrate and accept. Apply a removal test to every compound candidate: if one named journey or outcome can be removed while the remainder still has independent user value, split it into another Feature, even when both parts use the same actor, domain concept, screen, or technical foundation. A domain heading such as account, project, or task management is context, not by itself a Feature objective. Model a functional prerequisite as a dependency edge instead of using it to merge journeys. For example, authenticating, recovering access, and maintaining one's profile are separate journeys.

Keep CRUD operations together only when they are variants of one cohesive maintenance journey that the user would demonstrate, accept, and prioritize as a unit. Otherwise apply the removal test and split them. Split reusable prerequisites, independently acceptable actor goals/outcomes, and independently observable Bug symptoms. A suspected shared technical cause does not merge Bugs. For each item, use `separation_reason` to record why it is independently acceptable and why any operations retained inside it form one journey. Re-run the removal test after candidate mapping and before presenting the catalog.
...
**Complete when:** every candidate is decided and mapped; every active item passes the removal test and has a boundary rationale; every source is allocated once; the graph is valid and minimal; planned `blocks` relations exactly project its edges; one valid approval covers the complete projection; and no item-level functional interview has started.
