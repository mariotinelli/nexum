...
A [projeção completa do catálogo](catalog-time-review.md) contém todos os itens ativos e retirados, entregas, cobertura, fontes, dependências, justificativas e ordem. Estado candidato: [scope-state.time-review.json](../legacy/scope-state.time-review.json). Referência anterior: [scope-state.time-before.json](../legacy/scope-state.time-before.json).
