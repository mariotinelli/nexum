...
Uma tarefa pode ser criada, consultada, editada, atribuída e acompanhada por ações explícitas de situação, com comentários, imagens, anexos e histórico. A aba de tarefas e o progresso por quantidade ficam integrados ao projeto. As ações funcionam sem cronômetro antes da integração; Gestão de Tempo acrescenta a execução cronometrada e o registro de horas conforme R14.

## Atores e permissões

Todas as permissões exigem conta ativa. Além disso, exigem participação atual no projeto ou acesso administrativo. Projeto arquivado permite somente consulta de seu conteúdo; reativá-lo pertence à Gestão de Projeto.

| Operação | Quem pode executar | Limite |
| --- | --- | --- |
| Consultar tarefa e registros; baixar anexos | Qualquer participante ou administrador | Inclui finalizadas, canceladas e arquivadas |
| Criar e editar dados da tarefa | Gestores do projeto ou administradores | Projeto ativo; canceladas e arquivadas não admitem edição |
| Atribuir ou deixar sem responsável | Qualquer participante ou administrador | Projeto ativo; tarefa pendente, em andamento, pausada ou finalizada; nova atribuição somente a participante ativo |
| Iniciar, Pausar e Finalizar | Somente o responsável atual | Projeto ativo e ação permitida pela situação |
| Cancelar e Arquivar | Gestores do projeto ou administradores | Projeto ativo e ação permitida pela situação |
| Desarquivar | Gestores do projeto ou administradores | Tarefa arquivada em projeto ativo; retorna a Pendente |
| Comentar e anexar | Qualquer participante ou administrador | Projeto ativo; tarefa pendente, em andamento, pausada ou finalizada |
...
Ser autor da tarefa não concede privilégios permanentes. Ser responsável permite as ações indicadas, sem conceder edição dos demais dados. Administradores acessam e administram tarefas de qualquer projeto, mesmo sem participação. Iniciar/Pausar/Finalizar continuam exclusivos do responsável atual, inclusive para administradores; ninguém controla manualmente cronômetro alheio. Contas inativas não acessam o sistema.

## Histórias de usuário

1. Como gestor do projeto ou administrador, quero criar e editar uma tarefa para descrever trabalho, prioridade, datas e estimativa.
2. Como participante ou administrador, quero consultar detalhe e registros para acompanhar o trabalho do projeto.
3. Como participante ou administrador, quero atribuir tarefas a outro participante ativo, a mim ou deixá-las sem responsável para distribuir responsabilidades.
4. Como responsável, quero Iniciar, Pausar e Finalizar conforme a situação para representar o andamento.
5. Como gestor do projeto ou administrador, quero Cancelar, Arquivar e Desarquivar nas situações permitidas para administrar o ciclo de vida.
6. Como participante ou administrador, quero colaborar com comentários formatados, imagens e anexos e manter meus próprios registros.
7. Como participante ou administrador, quero consultar autoria e mudanças no histórico para entender a evolução da tarefa.
8. Como gestor do projeto ou administrador, quero receber aviso de edição concorrente para revisar os dados sem perder o texto digitado.
9. Como participante ou administrador, quero ver tarefas, atraso e progresso no projeto para acompanhar sua entrega.
10. Como gestor do projeto ou administrador, quero confirmar o impacto de remover um participante responsável para preservar os registros e retirar suas atribuições.

## Fluxo principal

**Criação:** o gestor do projeto ou administrador abre Nova tarefa pela lista ou projeto, seleciona um projeto ativo autorizado e informa Título. Pode preencher Descrição, Responsável, Data inicial, Prazo e Estimativa em horas. A tarefa inicia Pendente, prioridade Média e autor automático; pode escolher outra prioridade fixa. Após validar e criar, abrir o detalhe com confirmação. Cancelar o formulário não cria a tarefa e retorna à lista de origem.

**Consulta:** um participante ou administrador abre o detalhe pelo projeto ou pelas consultas que consumirem esta Feature. Apresentar título, projeto, descrição, situação, prioridade, responsável, autor, datas, estimativa e data de finalização quando existente. Oferecer comentários, área própria de anexos e histórico. Ações respeitam simultaneamente papel, situação da tarefa e estado do projeto.

**Edição:** o gestor do projeto ou administrador altera os dados atuais e salva. Projeto e autor permanecem fixos; situação não é campo editável. Validar e retornar ao detalhe com confirmação. Cancelar mantém os dados anteriores. Finalizadas continuam admitindo edição dos dados por gestores do projeto ou administradores, atribuição e colaboração, sem ações de situação.

**Atribuição:** qualquer participante ou administrador seleciona qualquer participante ativo do mesmo projeto, inclusive a si quando participante, ou Sem responsável. A mudança vale imediatamente para as ações do responsável e aparece no histórico, sem mudar as permissões de edição dos dados.
...
**Colaboração:** participantes e administradores publicam comentários com texto rico, consultam-nos em ordem cronológica com autor e horário e editam/excluem somente os próprios. Descrição e comentários permitem quebras de linha, negrito, itálico, listas, links e imagens no conteúdo. Anexos ficam separados da descrição: participantes e administradores adicionam e baixam arquivos e excluem somente os próprios mediante confirmação.
...
- Sem participação nem acesso administrativo, com conta inativa ou sem permissão: recusar a operação, inclusive por acesso direto ou tela previamente aberta. Reavaliar papel e atribuição atuais ao executar ações.
...
- Cancelada: somente consulta/download, exceto Arquivar por gestor do projeto ou administrador. Arquivada: somente consulta/download, exceto Desarquivar por gestor do projeto ou administrador em projeto ativo.
- Sem responsável: ninguém pode Iniciar, Pausar ou Finalizar até uma atribuição válida. Participantes e administradores podem atribuir conforme as regras gerais; gestores e administradores mantêm apenas as demais operações permitidas por seu papel e pela situação.
...
R1. Consulta depende de participação ou acesso administrativo; criação e edição dos dados são exclusivas de gestores do projeto e administradores. Atribuição e colaboração obedecem à matriz de permissões. Autor, responsável e administrador não recebem poderes adicionais aos descritos.

R2. Projeto e Título são obrigatórios. Projeto deve estar ativo e ser gerenciado pelo criador, ou o criador deve ser administrador. Autor é automático e imutável. Projeto não pode ser trocado depois da criação neste MVP.
...
R6. Nova atribuição exige participante ativo do mesmo projeto. Qualquer participante ou administrador pode atribuir a si quando participante ou a outro participante ativo, ou retirar atribuição, inclusive em finalizada. Responsável inativo existente permanece identificado até reatribuição ou remoção da participação. Remoção confirmada retira suas atribuições e preserva registros anteriores.

R7. Situação muda exclusivamente pelas ações da tabela. Somente o responsável atual executa Iniciar/Pausar/Finalizar. Um gestor ou administrador só executa essas três ações quando também for o responsável. Gestores do projeto e administradores executam Cancelar/Arquivar/Desarquivar. Somente ações válidas ficam disponíveis; outras transições são recusadas. Não há seletor de situação, mudança por arrastar, Bloquear/Bloqueada, Retomar, Reabrir ou Devolver à fila.

R8. Finalizar registra data/hora. Finalizada não permite ações de situação, inclusive Arquivar; mantém edição dos dados por gestores do projeto ou administradores, atribuição e colaboração conforme as permissões. Esses atos não alteram sua situação nem reabrem a tarefa.
...
R10. Comentários têm conteúdo obrigatório, autor e horário e são apresentados cronologicamente. Participantes e administradores editam/excluem somente os próprios; exclusão exige confirmação. Descrição e comentários usam editor rico com imagens e a formatação aprovada. Sem comentários privados ou menções.

R11. Anexos ficam em área própria, separada da descrição. Aceitar PDF, imagens, documentos de texto, planilhas e ZIP, até 20 MB por arquivo. Participantes e administradores adicionam/baixam e excluem somente os próprios com confirmação. A restrição de acesso à tarefa também protege seus arquivos; finalizadas mantêm colaboração, canceladas/arquivadas permitem apenas download dos anexos.
...
R14. As ações de tarefa funcionam sem cronômetro antes da integração. Com Gestão de Tempo, Iniciar inicia execução; Pausar salva horas e encerra a execução, deixando a tarefa Pausada e liberando outra tarefa. Finalizar salva o tempo ainda não registrado e finaliza a tarefa, sem exigir encerramento separado nem duplicar apontamentos de pausas anteriores; sem tempo contado, não cria apontamento. Não existe Descartar. Cronômetro rodando impede arquivar o projeto. Troca permitida de responsável, remoção de participante ou inativação pausa automaticamente e salva até o instante da alteração, sem finalizar tarefa. A integração respeita as transições da tabela e as ações exclusivas do responsável.
...
CA1 — Como gestor de projeto ativo ou administrador, ao criar com título e projeto válidos, gerar tarefa Pendente, prioridade Média e autor atual automático; abrir detalhe e confirmar. Membro sem acesso administrativo não pode criar; administrador pode criar sem participação. [R1–R3]
...
CA3 — Como gestor autorizado ou administrador, ao editar dados válidos, salvar e retornar ao detalhe; cancelar preserva dados anteriores. Trocar projeto/autor ou editar diretamente situação é recusado. Responsável sem papel Gestor nem acesso administrativo não edita demais dados. [R1–R3, R7]

CA4 — Como participante ativo, ao abrir tarefa do projeto, mostrar dados e registros permitidos. Sem participação nem acesso administrativo, impedir acesso à tarefa e anexos, inclusive por acesso direto ou tela já aberta. [R1, R11]
...
CA10 — Como responsável atual, ao executar Iniciar/Pausar/Finalizar nas origens autorizadas, permitir a ação. Gestor ou administrador que não seja responsável e demais participantes são impedidos; sem responsável, ninguém executa essas três ações até atribuição válida. Gestor ou administrador também responsável pode executá-las. Cancelar/Arquivar/Desarquivar são recusadas para quem não seja gestor do projeto nem administrador. [R7; exceções]

CA11 — Com Pendente, Em andamento ou Pausada, ao Finalizar, torná-la Finalizada e registrar data/hora. Depois, não oferecer ações de situação, inclusive Arquivar/Reabrir, mantendo edição por gestor do projeto ou administrador, atribuição, comentários e anexos. [R8]

CA12 — Com Em andamento, ao tentar Cancelar ou Arquivar diretamente, recusar. Após Pausar, permitir ao gestor do projeto ou administrador as ações previstas para Pausada. [R7]

CA13 — Com Cancelada, ao tentar editar, atribuir, comentar ou anexar, recusar; manter consulta/download e permitir ao gestor do projeto ou administrador Arquivar. [R9–R11]

CA14 — Com origem que permite Arquivar, ao confirmar como gestor do projeto ou administrador, arquivar, preservar registros e retirar das listas de trabalho, quadro e progresso. Cancelar a confirmação mantém situação e dados. [R9, R13]

CA15 — Com Arquivada em projeto ativo, ao Desarquivar como gestor do projeto ou administrador, retornar a Pendente, inclusive se era Cancelada antes. Em projeto arquivado, recusar Desarquivar e demais mutações, mantendo consulta/download. [R9; exceções]
...
CA19 — Ao excluir anexo próprio, pedir confirmação e remover após confirmar; cancelar preserva. Recusar excluir anexo alheio, inclusive por gestor ou administrador; canceladas/arquivadas permitem somente consulta/download dos arquivos. [R11]
...
CA25 — Sem integração de cronômetro, ao executar ação válida, mudar situação normalmente. Com Gestão de Tempo, Iniciar começa execução; Pausar salva horas, deixa Pausada e permite iniciar outra tarefa; Finalizar salva somente o tempo ainda não registrado e conclui a tarefa. Sem tempo contado, não gerar apontamento. Não exigir encerramento separado nem oferecer Descartar. Projeto com cronômetro rodando não pode ser arquivado. Troca permitida de responsável, remoção de participante ou inativação pausa e salva até a alteração, sem finalizar tarefa. [R14; integração]
